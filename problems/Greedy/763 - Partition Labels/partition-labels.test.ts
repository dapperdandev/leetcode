// 763. Partition Labels (Medium)
// https://leetcode.com/problems/partition-labels/
// NeetCode 150: Greedy
//
// You are given a string s. We want to partition the string into as many parts as possible so that each letter
// appears in at most one part. For example, the string "ababcc" can be partitioned into ["abab", "cc"], but
// partitions such as ["aba", "bcc"] or ["ab", "ab", "cc"] are invalid.
//
// Note that the partition is done so that after concatenating all the parts in order, the resultant string
// should be s.
//
// Return a list of integers representing the size of these parts.

import { describe, expect, it } from "vitest";
import { partitionLabels } from "./partition-labels.ts";

describe("763. Partition Labels", () => {
    it("example 1", () => {
        const s = "ababcbacadefegdehijhklij";
        expect(partitionLabels(s)).toEqual([9, 7, 8]);
    });

    it("example 2", () => {
        const s = "eccbbbbdec";
        expect(partitionLabels(s)).toEqual([10]);
    });
});
