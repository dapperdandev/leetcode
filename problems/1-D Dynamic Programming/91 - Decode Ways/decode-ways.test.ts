...
//
// Note: there may be strings that are impossible to decode. Given a string s containing only digits, return
// the number of ways to decode it. If the entire string cannot be decoded in any valid way, return 0.
//
// The test cases are generated so that the answer fits in a 32-bit integer.

import { describe, expect, it } from "vitest";
import { numDecodings } from "./decode-ways.ts";

describe("91. Decode Ways", () => {
    it("example 1", () => {
        const s = "12";
        expect(numDecodings(s)).toBe(2);
    });

    it("example 2", () => {
        const s = "226";
        expect(numDecodings(s)).toBe(3);
    });

    it("example 3", () => {
        const s = "06";
        expect(numDecodings(s)).toBe(0);
    });
});
