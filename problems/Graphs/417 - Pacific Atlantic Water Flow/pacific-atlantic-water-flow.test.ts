...
// Ocean touches the island's left and top edges, and the Atlantic Ocean touches the island's right and bottom
// edges.
//
...
import { pacificAtlantic } from "./pacific-atlantic-water-flow.ts";
...
describe("417. Pacific Atlantic Water Flow", () => {
    it("example 1", () => {
        ...
        expect(sortDeep(pacificAtlantic(heights))).toEqual(
            sortDeep([
                [0, 4],
                [1, 3],
                [1, 4],
    ...
    it("example 2", () => {
        const heights = [[1]];
        expect(sortDeep(pacificAtlantic(heights))).toEqual(sortDeep([[0, 0]]));
    });
});
